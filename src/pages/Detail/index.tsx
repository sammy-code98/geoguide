      ...
      <div className="px-4 sm:px-12 py-8">
        ...
        <div className="py-4 md:py-12">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-40 justify-items-center">
            ...
            <div>
              <h5 className="mb-2 text-3xl font-bold tracking-light text-black">
                {country?.name.common}
              </h5>
                ...
                <div className="space-y-4">
                  ...
                  <p className="font-normal text-black">
                    ...
                    {currency.map((curr: any) => (
                      <span
                        key={curr.name}
                        className="text-primary ml-1 capitalize"
                      >
                        {curr.name}
                      </span>
                    ))}
                  ...
                  <p className="font-normal text-black">
                    Currency Symbol :
                    {currency.map((curr: any) => (
                      <span key={curr.symbol} className="text-primary ml-1">
                        {curr.symbol}
                      </span>
                    ))}
                  </p>
                  ...
                  <p className="font-normal text-black">
                    UN Member :
                    {country?.unMember ? (
                      <span className="text-primary ml-1">Yes</span>
                    ) : (
                      <span className="text-primary ml-1">No</span>
                    )}
                  </p>
                </div>
              </div>
                    ...
                    <div className="grid grid-cols-4 md:grid-cols-12 gap-2 md:-ml-12">
                      {country?.idd?.suffixes.map(
                        (_: string | number, i: Key | null | undefined) => (
                          <div
                            key={i}
                            className="py-1 px-4 text-primary  text-sm"
                          >
                            {_}
                          </div>
                        )
                  ...
                  <div>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                      {country?.timezones.map((time: string) => (
                        <div
                          key={time}
                          className="py-1 px-2 text-primary border border-grayLight ml-2 rounded-md text-sm text-center"
                        >
                          {time}
                        </div>
                      ))}
                ...
                <div className="flex">
                  ...
                  <div className="ml-2">
                    {country?.altSpellings.map((alt: string) => (
                      <div key={alt} className="text-primary">
                        {alt}
                      </div>
                    ))}
                  </div>
